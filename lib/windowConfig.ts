...
const SIZE_PRESETS: Record<SizePreset, { width: number; height: number }> = {
    ...
    medium: { width: 480, height: 380 },
    large: { width: 0.7, height: 0.75 }, // 70% width, 75% height (will be calculated)
...
export function initializeWindowLayout(
    ...
    (Object.keys(WINDOW_CONFIGS) as WindowName[]).forEach((windowName) => {
        ...
        positions[windowName] = {
            x: position.x - cascadeOffset,
            y: position.y + cascadeOffset
        };
